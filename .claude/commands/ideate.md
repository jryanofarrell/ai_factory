...
2. **Ask, don't assume.** For each idea the user shares, ask at least one clarifying question before proposing anything. Good questions to ask:
   ...
   - For each file, identify the **skill** that captures the relevant pattern (or flag that the skill needs to be written first — see `/ticket`).
   - **Every subtask must end up executable by a small local model.** Each one runs on the local executor by default. That means by the end of decomposition every subtask has:
     ...
     - The skill it follows (read the skill during ideation if you haven't — don't guess at its contents).
     - The specifics the skill cannot know: exact column names, function signatures, branching algorithms in pseudocode, layout grids, copy strings, edge-case behavior.
   ...
   - **Read the skill files during the discussion**, not just their names. Codebase conventions (e.g., "all reads go through the manager", "BaseManager.get is findById, don't redefine it") are in the skills and easy to violate by pattern-matching from training data. Pulling the skill in catches violations before they become subtasks.

   The goal is that by the end of ideation, both you and the user can name the subtasks the ticket will contain, each one is detailed enough for a local model to execute, and every subtask follows its skill rather than inventing a new pattern.
...
- **The per-file subtask list** — one subtask per file, each pointing at the skill it follows, with a clear tier hint where the file is mechanical enough for the local tier
- **Any missing skills** that must be written before this ticket can be decomposed cleanly
