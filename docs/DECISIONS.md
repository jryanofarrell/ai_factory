...
The orchestrator groups its `work_items` into chains before the run loop, then iterates chains. Per-ticket Linear write-back is unchanged — each ticket gets its own comment with the shared PR URL and its own state transition. The simplifying v1 default: a dependency referenced in a queued ticket but not itself in the queue is treated as already-merged. The user gatekeeps by what they put in the queue. A future iteration could query Linear or the target repo's main branch to verify dep state strictly, gated on a manifest flag.
